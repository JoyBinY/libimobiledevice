...
LIBIMOBILEDEVICE_API diagnostics_relay_error_t diagnostics_relay_query_ioregistry_entry(diagnostics_relay_client_t client, const char* name, const char* class_0, plist_t* result)
{
	if (!client || (name == NULL && class_0 == NULL) || result == NULL)
		return DIAGNOSTICS_RELAY_E_INVALID_ARG;
	...
	if (class_0)
		plist_dict_set_item(dict,"EntryClass", plist_new_string(class_0));
